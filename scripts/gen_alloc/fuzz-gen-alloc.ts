...
async function main() {
    ...
    const indexOfShard = shardList.indexOf(from);
    ...
    let totalTxs = 0;
    while (true) {
        const activeWallet = walletsWithNonce[walletIndex];
        walletIndex++;
        if (walletIndex == walletsWithNonce.length) {
            walletIndex = 0;
        }
        let nonce = activeWallet.nonce;
        ...
        if (totalTxs % mempoolSize == 0) {
            try {
                ...
                console.log("current nonce", nonce, "at", Date.now());
                feeData = await provider.getFeeData();
                nonce = await provider.getTransactionCount(
                    activeWallet.address,
                    "pending"
                );
                console.log("new nonce", nonce, "at", Date.now());
            }
        }
        ...
        if (destination) {
            ...
            if (sendExternal) {
                ...
                receiveAddr = getRandomAddressInShard(from);
            }
        } else {
            ...
            if (sendExternal) {
                ...
                const shardAddr = addressList2[from];
                receiveAddr = shardAddr[Math.floor(Math.random() * shardAddr.length)];
            }
        }
        ...
        const signedTransaction = await activeWallet.wallet.signTransaction(
            rawTransaction
        );
